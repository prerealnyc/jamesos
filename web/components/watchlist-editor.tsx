...
// Watchlist editor — add / remove the creators & influencers we scrape.
// Lifted out of the old standalone /social-companion page so it can live
// INLINE on the Social Research page (the roster above reads from the same
// watchlist this writes to). `onWatchlistChange` lets a parent refresh the
// roster after the list is edited or a scrape is kicked.
export function WatchlistEditor({
  ...
  return (
    <>
      <Card>
        <div className="text-[13px] uppercase tracking-[1px] text-muted-foreground font-semibold mb-1">
          Watchlist — add the influencers we scrape
        </div>
        <p className="text-[12px] text-muted-foreground mb-3">
          New handles are picked up on the next weekly scrape — or hit{" "}
          <b>Refresh feed</b> to pull their recent posts right now.
        </p>
        ...
        {creators.length > 0 ? (
          ...
          <p className="text-[13px] text-muted-foreground mt-4">
            ...
            (Or use <b>Trend radar</b> below to discover by topic.)
          </p>
        )}
